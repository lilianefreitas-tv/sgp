            ...
            @foreach ($templates as $template)
                <article class="rounded-2xl border border-[#DCE3E7] bg-white p-5 shadow-sm">
                    <div class="flex items-start justify-between gap-3">
                        <div>
                            <p class="text-xs font-bold uppercase tracking-wider text-[#17A2B8]">{{ $template->code }}</p>
                    ...
                    <div class="mt-4 flex flex-wrap gap-2">
                        <span class="rounded-full bg-[#E7F3F6] px-3 py-1 text-xs font-semibold text-[#228A9D]">{{ $template->type->label() }}</span>
                    ...
                    <div class="mt-5 flex items-center justify-between border-t border-[#E8EDF0] pt-4">
                        <a href="{{ route('document-templates.edit', $template) }}" class="text-sm font-semibold text-[#228A9D] hover:underline">Editar</a>
