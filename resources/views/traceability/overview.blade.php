            ...
            @forelse($projects as $item)
                ...
                <a href="{{ route('projects.traceability.show', $project) }}" class="block rounded-2xl border border-[#DCE3E7] bg-white p-5 shadow-sm transition hover:border-[#8064A2] hover:bg-[#FBFAFD]">
                    <div class="flex flex-wrap items-start justify-between gap-4">
                        <div><p class="text-xs font-semibold text-[#17A2B8]">{{ $project->code }}</p><h3 class="mt-1 font-bold text-[#24313A]">{{ $project->name }}</h3><p class="mt-1 text-sm text-[#667680]">{{ $summary['requirements'] }} requisito(s) · {{ $summary['ready_cases'] }} caso(s) pronto(s) · {{ $summary['homologations'] }} homologação(ões)</p></div>
                    ...
                    <div class="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                        @foreach([
                            ...
                            <div class="rounded-xl bg-[#F6F8F9] px-4 py-3"><p class="text-xs font-semibold uppercase tracking-wide text-[#667680]">{{ $label }}</p><p class="mt-1 text-lg font-bold text-[#185063]">{{ $value === null ? 'N/A' : $value.'%' }}</p></div>
                        @endforeach
                    </div>
                </a>
            @empty
