            ...
            <tbody class="divide-y divide-[#E8EDF0]">
                @forelse($tasks as $task)
                    <tr class="text-sm text-[#24313A] {{ $task->is_active ? '' : 'bg-[#FAFBFB] opacity-75' }}">
                        <td class="max-w-md px-6 py-4"><a href="{{ route('projects.tasks.show', [$task->project_id, $task]) }}" class="font-semibold text-[#228A9D] hover:underline">{{ $task->code }} · {{ $task->title }}</a><p class="mt-1 text-xs text-[#667680]">@if($task->requirement){{ $task->requirement->code }} · {{ $task->requirement->title }}@else Sem requisito vinculado @endif</p></td>
                        @if($showProject)<td class="px-6 py-4"><p class="text-xs font-semibold text-[#17A2B8]">{{ $task->project->code }}</p><p class="mt-1 font-medium">{{ $task->project->name }}</p></td>@endif
                        ...
                        <td class="px-6 py-4 text-right"><a href="{{ route('projects.tasks.show', [$task->project_id, $task]) }}" class="font-semibold text-[#17A2B8] hover:underline">Visualizar</a></td>
                    </tr>
                @empty
