<x-app-layout>
    ...
    @endphp
    <div class="space-y-5">
        <section class="sgp-page-intro"><p class="sgp-page-kicker">Pipeline comercial</p><div class="mt-2 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><h2 class="text-2xl font-bold">Conduza oportunidades com histórico e rastreabilidade</h2><p class="mt-2 max-w-3xl text-sm leading-6 text-white/80">Centralize levantamentos, propostas, negociações e decisões antes da conversão em projeto.</p></div><a class="sgp-button-primary bg-white !text-[#185063] hover:!bg-[#EDF6F8] sm:w-auto" href="{{ route('initiatives.create') }}">Nova iniciativa comercial</a></div></section>
            ...
            <div class="divide-y divide-[#E8EDF0]">
                @forelse($initiatives as $initiative)
                    ...
                    <a class="grid gap-4 px-5 py-5 transition hover:bg-[#FBFCFD] md:grid-cols-[1fr_auto] md:items-center" href="{{ route('commercial.show', $initiative) }}">
                        <div><div class="flex flex-wrap items-center gap-2"><span class="text-xs font-bold text-[#17A2B8]">{{ $initiative->code }}</span><span class="sgp-badge {{ $state === 'won' ? 'sgp-badge-success' : ($state === 'lost' ? 'sgp-badge-danger' : 'sgp-badge-info') }}">{{ $stateLabels[$state] ?? 'Sem oportunidade' }}</span></div><h3 class="mt-2 font-bold text-[#24313A]">{{ $initiative->title }}</h3><p class="mt-1 text-sm text-[#667680]">{{ $initiative->opportunity?->code ?? 'Cadastre a oportunidade para iniciar o acompanhamento comercial.' }}</p></div>
