<x-app-layout>
    <x-slot name="header">
        <div>
            <h1 class="text-xl font-bold text-[#24313A]">Requisitos</h1>
            <p class="mt-1 text-sm text-[#667680]">Consulte os requisitos dos projetos aos quais você possui acesso.</p>
        </div>
    </x-slot>

    <div class="space-y-5">
        <form method="GET" action="{{ route('requirements.index') }}" class="grid gap-3 rounded-2xl border border-[#DCE3E7] bg-white p-5 shadow-sm sm:grid-cols-2 xl:grid-cols-[minmax(210px,1.35fr)_repeat(4,minmax(110px,0.85fr))_auto_auto] xl:items-end">
                    ...
                    <tbody class="divide-y divide-[#E8EDF0]">
                        @forelse ($requirements as $requirement)
                            <tr class="text-sm text-[#24313A] {{ $requirement->is_active ? '' : 'bg-[#FAFBFB] opacity-75' }}">
                                ...
                                <td class="px-6 py-4 text-right"><a href="{{ route('projects.requirements.show', [$requirement->project, $requirement]) }}" class="sgp-link">Visualizar</a></td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-14 text-center">
                                    ...
                                    <p class="mt-1 text-sm text-[#667680]">Ajuste os filtros ou acesse um projeto para cadastrar seu primeiro requisito.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
