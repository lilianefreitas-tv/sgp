        ...
        <section class="space-y-4">
            @forelse($projects as $project)
                ...
                <a href="{{ route('projects.tests.index', $project) }}" class="flex items-center justify-between gap-5 rounded-2xl border border-[#DCE3E7] bg-white p-5 shadow-sm transition hover:border-[#2E8B74] hover:bg-[#F7FBFA]">
                    <div><p class="text-xs font-semibold text-[#17A2B8]">{{ $project->code }}</p><h3 class="mt-1 font-bold text-[#24313A]">{{ $project->name }}</h3><p class="mt-1 text-sm text-[#667680]">{{ $project->test_cases_count }} casos · {{ $passed }}/{{ $ready->count() }} prontos com resultado aprovado · {{ $project->homologations_count }} decisões</p></div>
