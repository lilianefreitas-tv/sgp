...
class ApplicabilityEngine
{
    ...
    public function evaluate(ApplicabilityContext $context, iterable $rules): ApplicabilityResult
    {
        ...
        foreach ($rules as $rule) {
            ...
            $rawTargetType = $data['target_type'] ?? null;
            $targetType = $rawTargetType instanceof \BackedEnum ? $rawTargetType->value : $rawTargetType;
            if ($targetType !== null && $targetType !== $context->targetType->value) {
                continue;
            }
            if (isset($data['target_key']) && $data['target_key'] !== $context->targetKey) {
                continue;
            }
            if (($data['status'] ?? 'active') !== 'active' || ! $this->matches($context, $conditions)) {
                continue;
            }
            $matches[] = $data;
        }
        usort($matches, fn (array $a, array $b) => ((int) $b['priority']) <=> ((int) $a['priority']));
        if (isset($matches[1]) && $matches[0]['priority'] === $matches[1]['priority']) {
            throw new LogicException('Empate de prioridade em regras de aplicabilidade.');
        }
